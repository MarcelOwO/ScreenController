...
L2CapReceiver::L2CapReceiver()
    : l2_cap_socket_(-1), client_socket_(-1), buffer_() {
...
bool L2CapReceiver::init() {
  ...
  constexpr char val = static_cast<char>(0);
  loc_addr.l2_bdaddr = {{val, val, val, val, val, val}};
...
void L2CapReceiver::run() {
  ...
  if (poll(&fds, 1, 0) > 0 && (fds.revents & POLLIN)) {
    ...
    while (true) {
      buffer_.fill(static_cast<size_t>(0));

      const ssize_t len = read(client_socket_, buffer_.data(), buffer_.size());
      read_bytes += len;

      if (len > 0) {
        for (size_t i = 0; i < len; ++i) {
          received_data_.push_back(buffer_[i]);
        }
