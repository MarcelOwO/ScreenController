...
BluetoothDevice::BluetoothDevice(ILogger& logger,
                                 const std::shared_ptr<sdbus::IConnection>& connection,
                                 const sdbus::ObjectPath& device)
    : logger_(logger),
      device_proxy_(sdbus::createProxy(*connection, sdbus::ServiceName("org.bluez"), device)),
...
std::expected<void, std::error_code> BluetoothDevice::Disconnect() const {
  try {
    (void) device_proxy_->callMethod("Disconnect").onInterface(device_interface_);
    return {};
  } catch (const sdbus::Error& e) {
    logger_.LogError(e.getMessage());
    return std::unexpected(std::make_error_code(std::errc::operation_not_permitted));
...
std::expected<void, std::error_code> BluetoothDevice::ConnectProfile(std::string_view uuid) const {
  try {
    device_proxy_->callMethod("ConnectProfile").onInterface(device_interface_).withArguments(uuid);
    return {};
  } catch (const sdbus::Error& e) {
    logger_.LogError(e.getMessage());
    return std::unexpected(std::make_error_code(std::errc::operation_not_permitted));
...
std::expected<void, std::error_code> BluetoothDevice::CancelPairing() const {
  try {
    (void) device_proxy_->callMethod("CancelPairing").onInterface(device_interface_);
    return {};
  } catch (const sdbus::Error& e) {
    logger_.LogError(e.getMessage());
    return std::unexpected(std::make_error_code(std::errc::operation_not_permitted));
...
std::optional<std::vector<std::vector<uint8_t>>> BluetoothDevice::GetServiceRecords() const {
  try {
...
std::optional<std::string_view> BluetoothDevice::GetAddress() const {
  auto value =
      device_proxy_->getProperty("Address").onInterface(device_interface_).get<std::string>();
  if (value.empty()) {
...
std::optional<std::string_view> BluetoothDevice::GetAddressType() const {
  auto value =
      device_proxy_->getProperty("AddressType").onInterface(device_interface_).get<std::string>();
  if (value.empty()) {
...
std::optional<std::string> BluetoothDevice::GetName() const {
  auto value = device_proxy_->getProperty("Name").onInterface(device_interface_).get<std::string>();
  if (value.empty()) {
...
std::optional<std::string_view> BluetoothDevice::GetIcon() const {
  auto value = device_proxy_->getProperty("Icon").onInterface(device_interface_).get<std::string>();
  if (value.empty()) {
...
uint32_t BluetoothDevice::GetClass() const {
  const auto value =
      device_proxy_->getProperty("Class").onInterface(device_interface_).get<uint32_t>();
  return value;
}

uint16_t BluetoothDevice::GetAppearance() const {
  const auto value =
      device_proxy_->getProperty("Appearance").onInterface(device_interface_).get<uint16_t>();
  return value;
}
}  // namespace screen_controller::dbus
